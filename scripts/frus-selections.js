...
function applySelections(records) {
  ...
  for (const proposal of source.documents) {
    ...
    const record = {
      ...
      pageCount,
      extentLabel: `${pageCount} released memorandum ${pageCount === 1 ? 'page' : 'pages'}; PDF ${proposal.pdfPageStart === proposal.pdfPageEnd ? `page ${proposal.pdfPageStart}` : `pages ${proposal.pdfPageStart}–${proposal.pdfPageEnd}`}; attachments described separately`,
      naid: file.naid,
      localId: file.id,
      ...
      sourceNoteBasis: `Opening provenance marker (PDF page 1), selected memorandum pages, and terminal markings checked in the official NARA PDF on ${source.reviewedOn}.`,
      provenanceMethod: 'Opening PDF provenance marker and document source images',
      collectionId: existing?.collectionId || 'scowcroft',
